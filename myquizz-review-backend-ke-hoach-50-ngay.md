...
- [ ] **N19.6** (bổ sung ngoài kế hoạch): Session state một nguồn + gác đăng nhập + xóa màn Join + nút "Xem thêm" ở Home. **Kế hoạch chốt 6/9**
...
**Việc tách ra thành mốc riêng về sau**: màn **Discover** thật (feed cuộn vô hạn, nhiều section hơn Home). Hiện **chưa có mốc nào** trong kế hoạch — `Route.Discover` chỉ là placeholder không tới được. Backend `/feed` đã sẵn sàng (cursor + `hasMore` + cache 5 phút).
