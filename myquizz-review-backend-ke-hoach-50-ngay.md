...
- **N20.5** (mốc mới, đăng ký 6/9): màn **Discover** thật — feed cuộn vô hạn qua `GET /quizzes/feed` (cursor + `hasMore`), nhận sẵn `sectionKey` từ nút "Xem thêm" của Home, nhiều section hơn Home (kể cả section không có đích phân trang). Có thể trượt sau N21–25 nếu ưu tiên gameplay.
...
**Việc tách ra thành mốc riêng về sau**: màn **Discover** thật (feed cuộn vô hạn, nhiều section hơn Home) — đã đăng ký thành **mốc N20.5** (6/9). `Route.Discover(sectionKey)` hiện đã tới được từ nút "Xem thêm" nhưng màn còn là placeholder. Backend `/feed` đã sẵn sàng (cursor + `hasMore` + cache 5 phút).
