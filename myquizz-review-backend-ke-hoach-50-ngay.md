...
**10 điểm lệch — vá doc trước khi code phần liên quan (3 điểm cuối mới phát hiện 24/8 khi đối chiếu lại trực tiếp qua GitHub API):**
...
- **Nợ nhỏ để lại**: `onExit(message)` ở nav graph chỉ `popBackStack`, chưa hiển thị lý do bị buộc rời phòng (N19); `submitAnswer` chưa xử lý ack (N21+); `player_avatar`/`lives` chưa vào DTO (N19).
