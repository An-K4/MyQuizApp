...
>
> Cập nhật lần cuối: 10/9/2026, sau khi hoàn thành N20 (Host Lobby thật: sửa config realtime, chia sẻ mã/link và bắt đầu trận). Trạng thái mới hơn N20 xem blockquote đầu file kế hoạch chính.
...
- Bài học đến N20: ngoài các bẫy cũ, config server sau normalize mới là nguồn sự thật; baseline patch trong lobby phải là config hiện tại của phòng; lệnh socket không ACK phải được xác nhận bằng broadcast + timeout; thêm nhánh sealed class dùng chung phải rà mọi consumer; và phải đọc tới hàm ghi dữ liệu thật thay vì chỉ đọc handler/controller.
