...
>
> Cập nhật lần cuối: 13/9/2026, sau khi hoàn thành N20.5 (màn Khám phá thật: Paging 3, filter/sort/category và public-only browse). Trạng thái mới hơn xem blockquote đầu file kế hoạch chính.
...
- Bài học đến N20.5: ngoài các bẫy cũ, config server sau normalize mới là nguồn sự thật; baseline patch trong lobby phải là config hiện tại của phòng; lệnh socket không ACK phải được xác nhận bằng broadcast + timeout; thêm nhánh sealed class dùng chung phải rà mọi consumer; optional-auth không đồng nghĩa public-only; và mixed-case pagination meta phải được test bằng chính production Json.
