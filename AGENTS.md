...
>
> Cập nhật lần cuối: 5/9/2026, sau khi hoàn thành N19 (Player lobby: join phòng bằng mã + guest nickname) và fix bug response lồng ba cấp `data.session.session`.
...
- **Bottom Navigation thật đã có từ N19.5** — 5 tab: Trang chủ / Thư viện / Tham gia / Hoạt động / Hồ sơ. `TopLevelTab` + `MainBottomBar` ở `app/navigation/MainScaffold.kt`; `Scaffold` bọc **ngoài** `NavHost` trong `AppNavGraph`, bar chỉ hiện khi destination hiện tại `hasRoute` một trong 5 tab — nhờ vậy màn con (Search, QuizDetail, editor) và toàn bộ màn game tự động ẩn bar, không cần logic ẩn/hiện riêng ở từng graph. Đổi tab bằng `NavHostController.navigateToTab` (`popUpTo<Route.Home>` + `saveState`/`restoreState`).
